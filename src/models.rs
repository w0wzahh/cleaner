...
- Theme dropdown vertical alignment — it still sat a few pixels below the
  neighboring buttons. It's now a regular button with a painted arrow and
  a popup menu, sharing the exact widget path as the other header buttons —
  identical height, perfectly level.

## [2.12.1] - 2026-09-13
### Improved
- Two-column scan pages on every cleaning tool — setup options in a
  fixed left column, results card filling the rest. Result cards now
  have stat chips up top, a filter/sort toolbar, the list filling the
  available height, and a footer action bar pinned to the bottom.
- Storage tab: summary chips (drives / total / free) and a two-column
  drive grid on wide windows.
- System Cleaner gained an Export report button, matching other tabs.

### Fixed
- The theme dropdown in the header sat a few pixels low with its label
  on the wrong side — it now renders level with the buttons as
  "Theme: [picker] [Customize]".
